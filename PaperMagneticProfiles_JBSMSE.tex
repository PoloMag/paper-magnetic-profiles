...
\thanks{
An abridged version of this manuscript was presented at the ABCM International Congress of Mechanical Engineering (COBEM \colorbox{yellow}{Completar... cidade etc}).
}}
...
Fluid flow profiles can be more easily investigated  experimentally, as no changes in the fluid flow hardware are required. In particular, the effect of the \emph{duration} of the fluid blows has been extensively investigated \cite{bib:teyber17_exper,bib:nakashima18-influen-exp,FORTKAMP2018}. There appears to be a consensus in the literature that the cooling capacity of magnetic refrigerators can be increased by displacing the fluid during periods where the magnetic field is at its extreme values. 

In contrast, because of the complexities involved in designing and fabricating magnetic circuits, the applied magnetic field profiles (or simply the magnetic waveforms) are much less studied. Magnetic waveforms are usually investigated in terms of their synchronization with the fluid flow profile \cite{TUSEK20111507,bib:bjoerk11_amr}. \colorbox{yellow}{acho q cabe explicar brevemente o que os trabalhos de \cite{TUSEK20111507} e \cite{bib:bjoerk11_amr} fizeram...}. Trevizoli et al.~\cite{bib:trevizoli16_perfor_model} carried out a numerical comparison of the square wave (step change), sinusoidal and rectified cosine magnetic profiles. However, no systematic assessment of the effect of amplitude was performed nor did they investigate different geometries \colorbox{yellow}{nao estah claro o que vc quer dizer por geometries}.


\colorbox{yellow}{Fabio, acho que revisao bibliografica tem que ser expandida, trazendo trabalhos mais recentes em refrigeracao magnetica.}

\colorbox{yellow}{Tambem tem que trazer aqui para a revisao os nossos artigos que mediram/testaram/modelaram as valvulas.}
...
\nomenclature[bb]{$\beta$}{surface area density [\si{\squared\meter\per\cubic\meter}]}
...
\nomenclature[bf]{$\Phi$}{utilization}
...
The casing heat transfer term $\rate{q}\ped{csg}$ in \autoref{eq:78} is calculated solving the heat conduction equation in the regenerator casing \cite{bib:trevizoli16_perfor_model}. It can be neglected in some circumstances if an insulating casing material is assumed, which greatly simplifies the analysis. %More details are provided in \autoref{sec:results-discussions}.
...
\nomenclature[bt]{$\dtad$}{adiabatic temperature variation [\si{\kelvin}]}
...
\nomenclature[azt]{$\tau\ped{R}$}{ramp period in the magnetic ramp profile [\si{\second}]}
\nomenclature[azt]{$\tau\ped{M}$}{magnetization period in the magnetic ramp  profile [\si{\second}]}
\nomenclature[azo]{$\theta\ped{R}$}{ramp rate in the magnetic ramp profile}
...
\colorbox{yellow}{Fabio, a ultima frase do paragrafo acima nao  cabe aqui... quero dizer que a aplicacao deveria ter sido discutida na revisao bibliografica, a qual precisa ser expandida}
...
In the present analysis, two types of valves are considered. In the first approach, called \emph{Type B valves}, the  model proposed in Ref.~\cite{bib:cardoso16_trans} is used, assuming that the individual consumption of each valve is independent of frequency and blow fraction. The valve power  $\wvalve$ can be computed as:

\colorbox{yellow}{da mesma forma, o trabalho \cite{bib:cardoso16_trans} tem que constar da revisao bibliografica...}

\colorbox{yellow}{fica estranho o \textbf{primeiro} approach ser Type B e o \textbf{segundo} ser Type A...}
